...
export const Textarea = React.forwardRef<HTMLDivElement, TextareaProps>(
  (
    { variant, color, size, label, error, success, resize, labelProps, containerProps, shrink, className, ...rest },
    ref
  ) => {
    // 1. init
    ...
    const textareaColor = objectsToString(textareaVariant.colors.textarea[findMatch(valid.colors, color, 'blue')]);
    ...
    const labelColor = objectsToString(textareaVariant.colors.label[findMatch(valid.colors, color, 'blue')]);
...
Textarea.displayName = 'BlusteryUi.Textarea';
